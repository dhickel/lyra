...
| Nominal types (execution pending) | `NominalTypeIdTest` and `NominalTypeTest`: identity, invariant types and recursive schemas. `NominalSyntaxTest`: grammar/replay and malformed source. `NominalSemanticsTest`: schemas, privacy, constructors, initial typed/IR field and method flow, immutable initialization, incomplete-self escape and saved-slot/alias behavior. `TypedIrTest` retains exhaustive source-produced variant coverage including nominal declarations/construction | Four grammar seeds × 240 cases, four semantic field/signature models × 100 cases, and three independent branch-set initialization models × 36 cases (1,800 per seed in extended campaigns). These are syntax/semantic/IR checks, not JVM execution coverage. General constructor summaries, contextual replacement self, complete cyclic/imported/repeated heap transfer and JVM/session integration remain in progress. The excluded class fixture tests inheritance; the older public-API deferred-class negative now checks an actually uninitialized class. Track full gates in `.internal-dev/plans/nominal-types/plan.md`. |
