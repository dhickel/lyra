...
| Nominal construction authority (emitter wiring pending) | `NominalConstructionTest`: exact receiver/type, one-time ticket binding, partial reads/publication, immutable and mutable slots, zero-field structs, failed constructors/producers, private/public and foreign-domain access, owner-thread confinement | Two independent initialized-slot/value models × 80 cases (1,800 per seed extended). Fixtures store primitive values in exact Java fields; no production helper computes expected values. Source-to-JVM nominal construction remains a separate completion gate. |
| Nominal class plans (method-body emission pending) | `GeneratedTypePlannerTest`: source-derived exact class/field/member inventories, recursive linkage, constructor-to-instance signatures, empty structs, no type-name instance storage and forged layout rejection. `JvmAbiMapperTest` includes nominal-field scalar/nullable parity | The two nominal projection seeds also assert independent field, initialization and accessor descriptors and mutability (24 cases per seed, 1,800 extended). No source-to-JVM execution or unsupported-emission success is claimed. |
