...
| While syntax (execution work in progress) | Reserved unqualified bracket/parenthesized calls, compact/full/stored callbacks, nested iter/while syntax, conditional-arrow boundaries, rejected shadowing/qualified names and malformed delimiters | `LexerTest` includes the exact keyword inventory; `GrammarMatcherTest` covers positive and negative forms; `ParserTest` independently asserts two forms, no receiver, target name and argument count for generated trivia/callback/call spellings at seed 24301. `lyra.fuzz.cases` extends the default 180 cases. Runtime, type-contract and session models are still pending and must not be inferred from syntax coverage. |
...
| Range foundation (iteration work in progress) | Enclosed exclusive/inclusive construction, computed bounds, mandatory step, signed domain constraints, stored values and exact Java export signatures | `RangeIntegrationTest` includes a seeded independent integer traversal model, all signed widths, rejected domains/zero constants, runtime zero steps and terminal overflow boundaries; the grammar/parser suites and sealed IR inventory cover range syntax/lowering. Reserved `iter` has lexical, rejected-binding and call-boundary assertions, including a seeded parser generator varying trivia, endpoint forms and callback arities (extended by `lyra.fuzz.cases`). Executable `iter`, persistent snapshots and source-fuzzer composition are not yet complete. |
