...
| Nominal types (execution pending) | `NominalTypeIdTest` and `NominalTypeTest`: identity, invariant types and recursive schemas. `NominalSyntaxTest`: grammar/replay and malformed source. `NominalSemanticsTest`: schemas, privacy, constructors, typed/IR field and method flow, immutable initialization, incomplete-self escape, saved-slot/alias behavior, transitive factory calls and ordered constructor effects. `TypedIrTest` retains exhaustive source-produced variant coverage including nominal declarations/construction | Four grammar seeds × 240 cases, four semantic field/signature models × 100 cases, three independent branch-set initialization models × 36 cases, and three factory callable-slot ordering models × 24 cases (1,800 per seed in extended campaigns). These are syntax/semantic/IR checks, not JVM execution coverage. Contextual replacement self, complete cyclic/imported/repeated heap transfer and JVM/session integration remain in progress. The excluded class fixture tests inheritance; the older public-API deferred-class negative now checks an actually uninitialized class. Track full gates in `.internal-dev/plans/nominal-types/plan.md`. |
| Nominal runtime contracts (object execution pending) | `NominalTypeContractTest`: explicit closed schema parsing, recursive fields, data restrictions, unknown/duplicate/forged contracts. `JvmAbiMapperTest`: exact compiler/runtime nominal descriptor parity, nullable/array positions and wrong-family/origin negatives | Two recursive-schema seeds × 80 cases (1,800 per seed in extended campaigns). These are runtime metadata and physical mapping checks, not generated object execution or artifact/schema publication coverage. |
| Nominal artifact schemas (compiler object publication pending) | `NominalArtifactMetadataTest`: schema-2 recursive/export round-trips, identity/order/version/member corruption and full-schema revision binding; existing legacy/debug schema-1 tests remain unchanged | Two schema-publication seeds × 60 cases (1,800 per seed extended), varying recursive layouts and checking field-name tampering against an unchanged revision. These are metadata integrity checks, not live-object authority or emitted-object execution. |
...
| Producer-scoped nominal signatures | `NominalArtifactMetadataTest`: schema-1/2 compatibility, exact producer environment, wrong-thread and closed-owner rejection, failed initialization, and shared-cache identity | Two independent producer-lifetime seeds × 60 cases (1,800 per seed extended). Retiring one producer must reject its cached lookups without retiring another producer sharing the same metadata. These tests do not establish emitted nominal object execution. |
| Nominal construction authority (source factory pending) | `NominalConstructionTest`: exact receiver/type, one-time ticket binding, partial reads/publication, immutable and mutable slots, zero-field structs, failed constructors/producers, private/public and foreign-domain access, owner-thread confinement, and the active ticket's exact self-reference exception | Two independent initialized-slot/value models × 80 cases (1,800 per seed extended). Fixtures store primitive values in exact Java fields; no production helper computes expected values. Lyra construction-expression execution remains a separate completion gate. |
| Emitted nominal classes (source factory pending) | `GeneratedTypePlannerTest`: exact class/field/member/linkage inventories. `NominalBytecodeTest`: public compiler source input, deterministic final representation bytes, exact private primitive/reference fields, initialized/generated/public method bodies, owner/thread/lifecycle checks, same-class foreign-producer rejection, self links, nullable arrays, callable rejection, typed facade/function boundaries and ordinary artifact invocation | Two independent emitted-field seeds × 24 cases by default and 1,800 extended. The host drives authenticated tickets against source-produced classes until module factory and `IrNode.Construction` emission are wired; this is emitted bytecode execution, not a claim that Lyra construction expressions execute yet. |
