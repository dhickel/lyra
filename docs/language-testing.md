...
| Nominal types (execution pending) | `NominalTypeIdTest` and `NominalTypeTest`: declaration identity, invariant types, closed recursive schemas and malformed contracts. `NominalSyntaxTest`: grammar/replay, brackets and malformed source. `NominalSemanticsTest`: source-issued schemas, constructor parameters, private access, receiver captures, inferred receivers, nominal redeclarations and recursive data restrictions | Four grammar seeds × 240 cases and four independent semantic field/signature models × 100 cases (1,800 extended each), plus identity models. Keyword highlighting and console completeness are tested. These are resolver/syntax/type-model tests, not nominal execution coverage. Definite initialization, full field/callable flow, contextual replacements and IR/JVM/session integration remain in progress. The excluded class fixture tests inheritance. Track full gates in `.internal-dev/plans/nominal-types/plan.md`. |
