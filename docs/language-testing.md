...
| Functions | Contextual/inline signatures, compact argument lambdas, exact arity, non-callable rejection, direct/callable calls, named S/F result/effect/span parity, selected mutable receiver/member slots, guarded self-tail recursion, structured non-tail overflow, nilable-Fn coalesce and closure identity | `CallableCallParityTest` covers authenticated route classes, post-entry forgery rejection, normalized call/control-flow and exception-table parity, target/argument overflow frames (including the facade module frame only for `LYR-STACK`), dynamic authentication and per-instance signature metadata. `Phase15SmokeTest` pins ordinary facade failures to their source-level callee/caller frames and rejects a broad facade `LyraRuntimeException` catch. Nominal getter/adaptor coverage verifies that each generated nominal representation resolves its distinct callable member signatures exactly once per object in its constructor, and every generated getter, setter, initialization and delegated-read route boundary reads those private final fields instead of resolving or parsing per read while keeping the exact object/field-route/selected-candidate/signature/lifecycle authentication. The typed-program generator's `NAMED_CALL` and `MUTABLE_SELF_CALL` modes and `functions/named-call-parity.lyra` keep named-call and argument-time self-rebinding parity in default conformance. `LanguageCoverageTest` independently executes the mutable-selection model at every numeric width. Generated callable expressions/captured cells plus `Phase16SmokeTest`, `Domain11*Test` and `CallableSummaryTest` cover recursive linking, flow and ownership failures |
| Ranges and iter | Enclosed exclusive/inclusive construction, computed bounds, mandatory nonzero step, signed domains, reusable values, zero/one-argument Unit callbacks and exact Java exports | `RangeIntegrationTest` and `CallbackLoopIntegrationTest`: independent traversal models at all four widths, empty/directional/terminal boundaries, fresh captured parameters, stored/computed/imported callbacks, imported-mutation rejection across repetition, sealed IR inventory, persistent range snapshots and cross-submission reuse. The source-fuzzer `loops` mode composes both loops with an independent result/effect model and renders negative range components as explicit unary expressions so punctuation cannot create malformed fixtures. |
...
| Nominal types | `NominalTypeIdTest` and `NominalTypeTest`: identity, invariant types and recursive schemas. `NominalSyntaxTest`: canonical unprefixed `Type[...]`/`module->Type[...]` construction, legacy colon-prefixed replay, uppercase value indexing, grammar/replay and malformed source. `NominalSemanticsTest`: schemas, privacy, constructors, typed/IR field and method flow, immutable initialization, incomplete-self escape, saved-slot/alias behavior, transitive factory calls, ordered constructor effects, contextual replacement self and class-versus-struct identity typing. `TypedIrTest` retains exhaustive source-produced variant coverage including nominal declarations/construction. `RetainedNominalFlowCertificateTest` proves constructor-summary derivation for nested objects and tuple-routed arrays. `NominalBytecodeTest` executes construction, core member operations, receiver-bound replacements and cycle-safe equality. `NominalSessionTest` proves retained instances, methods, constructors, ordered arguments/defaults/bodies, captured and helper-returned closure installation, failure cleanup/source frames and snapshots across three submissions | Four grammar seeds × 240 cases, four semantic field/signature models × 100 cases, three independent branch-set initialization models × 36 cases, and three factory callable-slot ordering models × 24 cases (1,800 per seed in extended campaigns). The excluded class fixture tests inheritance; the older public-API deferred-class negative now checks an actually uninitialized class. |
| Retained nominal transfer/certification | `RetainedNominalFlowCertificateTest` and `NominalSessionTest` cover the closed initializer algebra, exact routes/provenance, per-construction fresh allocations, alias preservation, slot selection, failure publication, the flipped issue-#7 Unit/intrinsic inventory (exact Unit values, actual namespace-member callable invocation, exactly-once construction effects, occurrence-scoped route delegates, anti-laundering raw-before/raw-after/coexistence negatives, saved/captured/parameter/returned/rebound propagation, direct lambda capture/return/invocation recovery, field replacement with saved selection, selected-closure identity for repeated and routed/raw reads, nested-aggregate/private boundaries, later-generation use), `NominalMemberDelegateAuthorityTest` pins opaque single-use compiler-issued evidence, flat identity-deduplicated route dependencies under 10,000 alternating read/reassign operations, exact object/field/value/signature, writable-field caller/replacement authority, private/immutable fields, foreign-session/root, initializing/failed/closed source and replacement producers, root-close and retired-epoch negatives at the runtime boundary, and nilable member-read contracts (annotation, coalesce, predicate narrowing, `#NIL` value match) derived independently from the exact schema slot across generations | The compiler fuzzer's retained family is a deterministic three-generation session model: a producer generation declares nominals with member initializers from every legal transfer variant (literals, immutable/mutable references, lambdas, direct/namespace/callable calls, arrays/tuples, operators/conversions/ranges, conditionals/coalesce/match with guards, blocks with declarations/rebinding/shadowing, projections/indexing/length, nested construction, iter/while loops, nilable member references and `#NIL` values); a construction generation instantiates the factories through the authenticated storage domain; observation generations read members, identities (`eq?`), saved-versus-current slots, shadowed factories, nilable member reads through annotation/coalesce/narrowing/`#NIL` match, and expected failures. Independent plain-Java arithmetic supplies every expected value; forged routes, forged/mismatched inventories and mismatched link capabilities must be rejected; the fail-closed preflight guard must stay silent on legal source; the four pinned Unit/intrinsic shapes must return exact Unit values, preserve exactly-once construction effects, and invoke the retained namespace member where applicable. Ten profiles rotate deterministically, so the minimum 120-case budget per seed executes every profile, every operation name, and every pinned shape |
...
12. **Retained nominal transfer/certification:** every case drives a real
    ...
    recovery, the four pinned issue-#7 Unit/intrinsic observation shapes
    (exact Unit values and one actual namespace-member callable invocation
    across declaration, construction and later observation, with exactly-once
    construction effects), and negative certification evidence: forged object routes, forged/mismatched
    ...
    Route-delegate emission is session-only: `SessionCompilerTest` proves the
    exact shared delegate class inventory for callable nominal members and the
    unchanged AOT class inventory, while the flipped inventory and the
    anti-laundering/coexistence/replacement/propagation regressions live in
    `NominalSessionTest` and `NominalMemberDelegateAuthorityTest`.
    Nilable member reads (issue #8) now derive their contracts independently
    from the exact resolved schema slot at typed semantic provenance sealing
    and IR validation: annotated reads, coalescing, predicate narrowing and
    `#NIL` value match compile and execute across generations under current
    nil rules, while nilable receivers, mismatched annotations, forged member
    links and invented match narrowing stay structured failures and valid
    cases never reach `LYC-IR-003`; retained nil provenance is accepted at the
    IR boundary only through the compiler-issued session certificate.
...
For expression examples, remember that parentheses normally denote callable
application. The one preservation exception is an exact parenthesized direct call,
so `(::f[])` remains the direct call; `(::f[] extra)` still calls the value returned
by `f`. Whitespace alone does not stop a postfix accessor. In a comma-capable
...
Use `(f)` for a zero-argument callable-value call. In module/block sequences the
same narrow comma exception may separate a following `::` sibling; it is not general
