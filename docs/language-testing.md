...
| Nominal types (execution pending) | `NominalTypeIdTest`: declaration identity and independent hash/ordering model. `NominalSyntaxTest`: declarations/members/constructors, named types, bracket ambiguity, exact spans, malformed syntax and corrupted replay descriptors; real invalid value-index and unknown-type diagnostics | Four independent grammar seeds × 240 default cases (1,800 extended), plus identity models. Keyword highlighting and console delimiter completeness are tested. These are syntax/identity tests, not nominal execution coverage; semantic/IR/JVM/session integration is unfinished. The excluded class fixture now tests inheritance, not rejection of accepted class syntax. Track full gates in `.internal-dev/plans/nominal-types/plan.md`. |
