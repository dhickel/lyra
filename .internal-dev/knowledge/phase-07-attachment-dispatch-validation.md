...
- **Lyra grammar gotchas for test sources.** (1) `::name[args]` after any expression form in the same block is parsed as a *member* direct call on that expression (postfix `::`); standalone direct calls parse in fresh expression positions (initializer RHS, branch body, first form, or after a `let`), with the narrow exception that a comma may separate a sibling whose head begins with `::`. The exception also applies where marker-free match/cond forms would otherwise absorb that sibling; it is not general comma punctuation. (2) `Unit` is falsey and assignment is Unit-typed, so assignment-as-predicate tricks are not truthy side-effect expressions. (3) `(name args)` is a callable-value call with real recursion frames; only `::name[args]` participates in direct self-tail loop lowering.
