...
- Named-call parity exposed a distinct retained-binding boundary: an unqualified declaration from a prior submission is `DeclarationKind.EXTERNAL` and uses `$lyra$sessionAccessor`, not `IrSessionExecution.ExternalAccess`. Applying the dynamic current-caller authentication used for opaque callables can therefore reject a valid prior source-root closure when the producer graph intentionally fails `Linkage.sourceLocal` (for example generated root plus only intrinsic `std/io`). Do not fix this by widening `sourceLocal`; represent the exact compiler-certified external accessor as a separate route.
- `SessionFlowCertificate.certifiesBinding` alone is not enough to omit per-call authentication. A retained top-level binding can itself contain a raw imported/intrinsic closure that was valid inside its producer artifact but must remain rejected in a later importing generation. External storage-route proof must additionally establish root callable provenance. The implemented certificate carries only lambda identities whose summaries belong to each source-root generation; intrinsic/imported/opaque alternatives stay dynamic. Keep the `NominalSessionTest` raw-before/raw-after anti-laundering cases beside positive editor and REPL retained-call tests.
