...
- PTY drivers must own their whole process tree. The REPL child makes itself a session leader (`setsid`), so the driver's teardown kills the process group and sets `PR_SET_PDEATHSIG` in the child; the Java side force-kills driver descendants before the driver itself. Otherwise a timed-out or interrupted PTY test force-kills the python driver (skipping its `finally`) and orphans a live console REPL.
- Distribution-jar tests must fail fast on staleness. The packaged CLI jar can be older than the current console/command migration; a non-TTY `\help`/`\quit` probe against the real jar fails with the actual REPL output instead of a PTY timeout, and its teardown destroys descendants.
- Editor adapters may return a cooked line terminator even when the terminal provider already exposed one. Normalize that framing before command parsing and remove the console framing newline so `\type` receives exactly the submitted Lyra source, including trailing spaces.
