...
- The obsolete special-form spellings `::match[...]`, `::iter[...]` and `::while[...]` must be rejected in *every* position, including qualified (`ns->::match[...]`) and receiver-postfix (`x::match[...]`) readings, with `LYC-PARSE-012`. The receiver case used to return the base expression and let the next `::` start a fresh form; that "fresh expression" reading is gone, so the postfix, namespace-suffix, and unqualified-direct-call paths all need the explicit rejection together.
