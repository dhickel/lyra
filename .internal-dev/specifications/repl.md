...
- A separate typed session link domain validates exact scalar contracts, declaration/storage identity, assignment permission, initialization, revision and producer lifetime before any new source form executes. A later named call may omit redundant invocation authentication only when the retained external binding is compiler-certified and the runtime link table supplies its exact initialized typed accessor; otherwise the dynamic callable boundary remains mandatory. Matching names, types or numeric identity ordinals alone identify no storage. Ordinary AOT artifact compatibility, imported-module mutation rules, generated closure behavior and foreign-SAM rejection remain unchanged; optional trusted attachment does not alter those ordinary contracts.
...
The only developer commands are `\help`, `\bindings`, `\type`, `\load`, `\reload MODULE`, `\reset`, `\history` and `\quit`; command recognition reserves a single leading backslash at the existing top-level command boundary. Every colon-leading input unit is ordinary Lyra source: `:Type[arguments]` remains a compatibility spelling for nominal construction, while `::name[arguments]` direct calls and `:.` value access are also submitted to the compiler. Canonical nominal construction is `Type[arguments]`; a submission beginning with a colon is classified, completeness-checked, and analyzed without ever executing it. The former colon command aliases are removed rather than retained. `\type` never evaluates. Commands are recognized only at the top-level command boundary, never inside pending multiline source, a string, a comment, or program `readLine` input. History is in memory by default and optional bounded source-only files are client-controlled; source history never contains program input or protocol traffic. A successful `\load` contributes the exact loaded source only when the adapter returns that source; it never contributes the command/path, and attached protocol-v2 loads intentionally return no source text. A successful `\reset` clears client source history, while failed or busy reset attempts preserve it.

Plain, JLine, attached and editor surfaces share this one classification: the same backslash command set, the same non-executing `\type`/analysis behavior, the same bounded argument rules, completion and highlighting, and the same source-only history. Protocol v2 operation schemas are unchanged by the command-prefix migration; no remote operation is added or renamed for it. Source entry and local `readLine` share one coordinated input owner; attached evaluation retains host stdin/stdout/stderr and never captures process output into the socket result. Completion is bounded read-only metadata/filesystem lookup and is not a public editor API.
