...
The only developer commands are `\help`, `\bindings`, `\type`, `\load`, `\reload MODULE`, `\reset`, `\history` and `\quit`; command recognition reserves a single leading backslash at the existing top-level command boundary. Every colon-leading input unit is ordinary Lyra source: `:Type[arguments]` nominal construction, `::name[arguments]` direct calls and `:.` value access are all submitted to the compiler, and a submission beginning with a colon is classified, completeness-checked, and analyzed without ever executing it. The former colon command aliases are removed rather than retained. `\type` never evaluates. Commands are recognized only at the top-level command boundary, never inside pending multiline source, a string, a comment, or program `readLine` input. History is in memory by default and optional bounded source-only files are client-controlled; source history never contains program input or protocol traffic. A successful `\load` contributes the exact loaded source only when the adapter returns that source; it never contributes the command/path, and attached protocol-v2 loads intentionally return no source text. A successful `\reset` clears client source history, while failed or busy reset attempts preserve it.
