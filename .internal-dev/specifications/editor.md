...
Run saves open buffers, validates the selected source and creates a fresh editor-owned JVM. That process owns a `LyraSession`, a controller and the existing protocol-v2 server. Its source roots configure discovery only. An explicit file load initializes the selected source once; the selected top-level function is invoked through ordinary Lyra call syntax and compiler-certified session linkage. The editor console uses the same backslash developer commands as the plain/JLine consoles (`\help`, `\bindings`, `\type`, `\load`, `\reload`, `\reset`, `\history`, `\quit`); colon-leading input is Lyra source, and command classification, completeness checking and analysis never execute it. Further REPL submissions reuse initialized storage. Private top-level functions are in session scope; nested functions still require their enclosing lexical environment.
...
The GUI exchanges only the existing bounded protocol results and passive source metadata. No live generated value or owner thread is moved into JavaFX. Source acquisition, analysis, file traversal, execution, control and output delivery do not share a blocking UI execution path. Program stdin is separate from REPL input/history. Source history records the exact submitted source unit, including exact loaded source only when the console adapter returns it; it never records a `\load` command/path, and protocol v2 intentionally returns no loaded-source text. A successful `\reset` clears editor source history; failed or busy reset attempts do not. Output/history and filesystem traversal are bounded. Cooperative cancellation is identity-specific; Stop terminates only the owned child process. Parent death retires orphaned children.
