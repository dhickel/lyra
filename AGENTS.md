...
The repository contains a verified compiler/runtime/CLI backend foundation plus an optional REPL module. Plain/JLine consoles, owner-confined lifecycle, non-executing `:type`, authenticated loopback protocol, exact cross-submission scalar/data-aggregate storage, compiler-certified source-local callable persistence, shared structural JVM types, executed bounded snapshots and generated session cancellation boundaries are tested. Data aggregates may recursively contain scalars, arrays, tuples and certified callable values; imported-module persistence, reload, configured roots, asynchronous owner execution, coordinated program input, application attachment and debug deployment remain incomplete. The delivered REPL slices are not completion of the full REPL specification. A separate runtime bridge accepts exact callable values between OPEN source-local generations in one authenticated session epoch through existing generated Java export signatures, complementing the compiler-side named callable path.

## Current Baseline and Direction

The planning baseline is commit `a16a46b71780206c6cec27ee87adef29e5333069`; the current worktree contains substantial uncommitted backend work that must be preserved. The verified implemented pipeline is:
