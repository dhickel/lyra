...
**Accessor Operators:**
- `:.` - Value/field and bound-method access (with implicit self for methods)
- `::` - Direct invocation of resolved function or method names; never prefix compiler-recognized built-ins
- `->` - Namespace access
- Callable values use S-expression application; compiler-recognized built-ins use their bare names in both S-expression and F-expression forms
...
## Language and Lowering Invariants

- Compiler-recognized built-in operations must use a bare built-in head. Their F-expression spelling must never be prefixed with `::`; reserve `::` for direct invocation of resolved source function or method names. Every new or changed built-in must include a negative regression that rejects its `::builtIn[...]` spelling.
- Equivalent S-expression and F-expression calls are syntax alternatives, not separate execution features. When resolution proves the same exact target and no member/namespace dispatch-table lookup is required, both forms must converge on the same typed semantic operation, typed IR, JVM lowering, evaluation order, diagnostics, source mapping, tail-call behavior, and runtime result.
- When a member or namespace lookup genuinely requires a different access path, preserve the `:.` value-access versus `::` direct-invocation distinction while keeping externally observable call semantics aligned. Never infer call authority from a matching name, JVM descriptor, generated class, or structural type alone.
- Any syntax or built-in change must be completed across lexer, grammar descriptors, parser replay, AST visitors, resolution, typing, structural context, flow/provenance certification, sealed typed IR, JVM emission, sessions, editor/console tooling, conformance inventories, and fuzz models as applicable. A form is not complete merely because it parses or executes in one surface.
- Add paired S-expression/F-expression parity tests for values, side effects, failures, malformed input, mutable target selection, recursion/tail positions, source spans, and generated bytecode. Preserve counterexamples as permanent regressions.
- For compiler-proven equivalent calls, structural bytecode tests must reject avoidable per-invocation parsing, metadata reconstruction, boxing, or redundant authentication. Changes to call or built-in lowering also require representative performance evidence; do not accept a semantically correct path that regresses materially against its equivalent spelling.
- Dynamic callable, member, imported, and session-linked paths must retain exact type, route, owner, lifecycle, and authentication checks wherever compile-time proof is insufficient. Performance work must remove redundant checks only, never weaken an authority boundary.
- Independent semantic, flow/provenance, and IR validators must understand every new expression and value route, including nilable and retained-session cases. Expected source failures must remain structured diagnostics and must not escape as compiler invariants or unchecked backend exceptions.
- Do not place static aliases on a type interface when initialization depends on one of that interface's implementations. Primitive and built-in constants belong to an initialization-safe owner, and isolated initialization-order tests are required when changing such boundaries.
