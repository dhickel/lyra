...
/** An immutable type contract carrying the legal {@code @mut}/{@code @nil} qualifiers. */
public final class QualifiedType implements LyraType {
    ...
    public QualifiedType(LyraType baseType, Set<TypeQualifier> qualifiers) {
        ...
        if (qualifiers.isEmpty()) {
            throw new IllegalArgumentException("a qualified type needs at least one qualifier");
        }

        EnumSet<TypeQualifier> combined = EnumSet.noneOf(TypeQualifier.class);
        for (TypeQualifier qualifier : qualifiers) {
            combined.add(Objects.requireNonNull(qualifier, "qualifiers must not contain null"));
        }
        LyraType unqualified = baseType;
        while (unqualified instanceof QualifiedType nested) {
            combined.addAll(nested.qualifiers);
        ...
        this.qualifierList = List.of(TypeQualifier.MUT, TypeQualifier.NIL).stream()
                .filter(this.qualifiers::contains)
                .toList();
    ...
    /** Qualifiers in the canonical {@code @mut}, then {@code @nil} order. */
    ...
    public LyraType withQualifiers(Set<TypeQualifier> additionalQualifiers) {
        ...
        for (TypeQualifier qualifier : additionalQualifiers) {
            combined.add(Objects.requireNonNull(
                    qualifier, "additionalQualifiers must not contain null"));
        }
        return QualifiedType.of(baseType, combined);
    }
    ...
    /** Validates only the qualifier legality of this contract at a known position. */
    public void validateFor(TypePosition position) {
        Objects.requireNonNull(position, "position");
        if (hasQualifier(TypeQualifier.MUT) && !position.permitsMutableQualifier()) {
    ...
    public boolean equals(Object other) {
        ...
        if (!(other instanceof QualifiedType qualified)) {
            return false;
        }
        return baseType.equals(qualified.baseType) && qualifiers.equals(qualified.qualifiers);
    }
