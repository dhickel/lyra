...
public final class LexerTest {
    ...
    private static void currentVocabularyAndDeferredWords() {
        String source = "let import as I32 Array Tuple Fn Bool Char String Unit "
        ...
        List<TokenKind> expected = List.of(
                TokenKind.LET, TokenKind.IMPORT, TokenKind.AS, TokenKind.TYPE_NAME,
        ...
        check(lexed.tokens().get(11).modifier().orElseThrow() == ModifierKind.PUBLIC,
                "@pub carries decoded modifier metadata");
        check(lexed.tokens().get(12).modifier().orElseThrow() == ModifierKind.MUTABLE,
                "@mut carries decoded modifier metadata");
        check(lexed.tokens().get(13).modifier().orElseThrow() == ModifierKind.NILABLE,
